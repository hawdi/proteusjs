'use strict';

// Payload for "log" events
class ServerLog {
  constructor(event) {

    this.object = 'server';
    this.event = 'log';

    this.timestamp = event.timestamp;
    this.tags = event.tags;
    this.data = event.data;
    this.pid = process.pid;
  }
}

// Payload for "request" events via request.log
class RequestLog {
  constructor(request, event) {

    this.object = 'server';
    this.event = 'request';

    this.timestamp = event.timestamp;
    this.tags = event.tags;
    this.data = event.data;
    this.pid = process.pid;
    this.id = request.id;
    this.method = request.method;
    this.path = request.path;
  }
}

// Payload for "ops" events
class Ops {
  constructor(ops) {

    this.object = 'server';
    this.event = 'ops';

    this.timestamp = Date.now();
    this.host = ops.host;
    this.pid = process.pid;
    this.os = {
      load: ops.osload,
      mem: ops.osmem,
      uptime: ops.osup
    };
    this.proc = {
      uptime: ops.psup,
      mem: ops.psmem,
      delay: ops.psdelay
    };
    this.load = {
      requests: ops.requests,
      concurrents: ops.concurrents,
      responseTimes: ops.responseTimes,
      sockets: ops.sockets
    };
  }
}

// Payload for "response" events
class RequestSent {
  constructor(reqOptions, resOptions, request) {

    this.object = 'server';
    this.event = 'response';

    const req = request.raw.req;
    const res = request.raw.res;

    this.timestamp = request.info.received;
    this.id = request.id;
    this.instance = request.connection.info.uri;
    this.labels = request.connection.settings.labels;
    this.method = request.method;
    this.path = request.path;
    this.query = request.query;
    this.responseTime = Date.now() - request.info.received;
    this.statusCode = res.statusCode;
    this.pid = process.pid;
    this.httpVersion = request.raw.req.httpVersion;
    this.source = {
      remoteAddress: request.info.remoteAddress,
      userAgent: req.headers['user-agent'],
      referer: req.headers.referer
    };
    this.route = request.route.path;

    this.log = (request.route.settings.log === false ? [] : request.getLog());

    if (reqOptions.headers) {
      this.headers = req.headers;
    }

    if (reqOptions.payload) {
      this.requestPayload = request.payload;
    }

    if (resOptions.payload && request.response) {
      this.responsePayload = request.response.source;
    }
  }
}

// Payload for "request-error" events (whenever an Internal Server Error (500) error response is sent)
class RequestError {
  constructor(request, error) {

    this.object = 'server';
    this.event = 'error';

    this.timestamp = request.info.received;

    this.id = request.id;
    this.url = request.url; //will contain info of request url
    this.method = request.method;
    this.pid = process.pid;
    this.errorMessage = error.message; //check for error message
    this.errorStack = error.stack; //error stack
  }
}

module.exports = {
  ServerLog,
  RequestLog,
  Ops,
  RequestSent,
  RequestError
};
